...
def get_api_key() -> str:
    """
    Fetches the newsapi API key from environment variables.
    """
    return os.environ.get('NEWSAPI_KEY')

def get_news(tts_enabled: bool) -> None:
    """
    This function fetches the weather data from newsapi using
    API key from environment variables and stores the data in 
    "news.json". New Notification and info log is created each time 
    new data is fetched.
    """
    api_key = get_api_key()
    country = 'in'

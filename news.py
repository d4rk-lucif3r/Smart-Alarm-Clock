...
def get_api_key() -> str:
    """
    This function fetches the NewsAPI API key
    from environment variables.
    """
    api_key = os.getenv('NEWSAPI_API_KEY')
    if not api_key:
        raise ValueError("API key for NewsAPI not found in environment variables.")
    return api_key
...
def clearNews():
    ...
    "news.json" each time the reset button on UI
    is pressed.
    """
    clearAllNews = []

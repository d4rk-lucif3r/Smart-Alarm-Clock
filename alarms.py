...
def add_alarm(alarm: dict) -> list:
    """
    This function reads all existing alarms from the alarms.json file,
    appends each one to a list, appends the new alarms to the same list
    and writes that list to the same json file.
    """
    if filename.is_file():
...
def delete_alarm(alarmToBeDeleted):
    ...
    The for loop iterates through each alarm. If an alarm time is found
    that is not equal to the time the user has requested to delete,
    that alarm object is appended to a new list 'new_alarm_objects'.
    The edited list of alarms is then written to the json file and the
    start alarm thread is reset.
    """
    new_alarm_objects = []
    with open(tmpdir + '/alarms.json', 'r') as alarms_file:
        try:
...
def start_alarm() -> None:
    ...
    the alarm and adds each alarm to the scheduler.
    This function runs on a separate thread each time the user loads the program.
    """
...
def alarm_end(alarm_time: datetime.datetime, alarm: dict, news_call: bool, weather_call: bool) -> None:
    ...
    If the user has checked news and weather checkboxes then news 
    and weather information will be played. Covid Data News will be played 
    on every alarm.
    """
    alarm_notification = {
...
def clearAlarms():
    """
    This function clears alarms.json file's content when reset button 
    is clicked on UI.
    """
    clearAllAlarms = []
    with open(tmpdir + '/alarms.json', 'w') as alarms_file:

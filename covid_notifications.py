...
def get_covid_data(tts_enabled: bool):
    ...
    get method in requests module, creates a dictionary and stores it 
    in "covid_notifications.json" file. New Notification and info log 
    is created each time new data is found.
    """
    ...
    new_covid_notification = {
        ...
        'type': 'Covid News',
        'title': "Covid News for Delhi with total confirmed cases " +
...
def clear_covid_news():
    ...
    "covid_notifications.json" each time reset button on UI 
    is pressed.
    """
    new_covid_data = []
    with open(tmpdir + '/covid_notifications.json', 'w') as covid_file:

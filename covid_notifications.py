...
def get_covid_data(tts_enabled: bool):
    ...
    the get method in requests module, creates a dictionary and stores it 
    in the "covid_notifications.json" file. New Notification and info log are created each time new data is found.
    """
    ...
    with open(tmpdir + '/covid_notifications.json', 'w') as covid_file:
        ...
        new_covid_notification = ({
            ...
            'title': "Covid News for Delhi" 
                     + " with"
                     + " Total confirmed cases " + str(new_covid_data['total']['confirmed'])
                     + " Today's confirmed cases " + str(new_covid_data['delta']['confirmed'])})
        ...
        if tts_enabled:
            try:
                tts("Covid News for Delhi with total confirmed cases " 
                    + str(new_covid_data['total']['confirmed']) 
                    + " and today's confirmed cases " 
...
def clear_covid_news():
    ...
    "covid_notifications.json" each time the reset button on the UI 
    is pressed.
    """
    new_covid_data = []
    with open(tmpdir + '/covid_notifications.json', 'w') as covid_file:

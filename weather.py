...
def get_weather(tts_enabled: bool) -> None:
    ...
    API key from environment variables and stores the data in 
    "weather.json". New Notification and info log is created each time 
    new data is fetched.
    """
    # location
    location = 'New Delhi'
...
def clearAllWeather():
    ...
    "weather.json" each time reset button on UI 
    is pressed.
    """
    clearWeather = []
    with open(tmpdir + '/weather.json', 'w') as weather_file:

...
def get_api_key() -> str:
    ...
    from the environment variable since edict.json is deprecated.
    """
    return os.environ.get('OPENWEATHERMAP_API_KEY', 'default_api_key')


def get_weather(tts_enabled: bool) -> None:
    """
    This function fetches the weather data from openweathermap using
    api key provided in the environment variables and stores the data in 
    "weather.json".
    ...
    weather_log = ({
        'timestamp': time.strftime('%H:%M:%S'),
        'type': 'weather',
        'description': 'Current Weather in ' + new_weather['name'] +
                       ' has been updated.',
        'error': ''
    })

...
Openweathermap using api key fetched from environment variables
"""
...
def get_weather(tts_enabled: bool) -> None:
    ...
    "weather.json". New Notification and info log are created each time new data is fetched.
    """
    # location
    location = 'New Delhi'
    api_key = get_api_key()
    if not api_key:
        raise ValueError("API key is missing. Make sure to set the environment variable 'OPENWEATHERMAP_API_KEY'.")
    ...
    weather_notification = ({'timestamp': time.strftime('%H:%M:%S'),
                             'type': 'Weather',
    ...
    if tts_enabled:
        try:
            tts('Current temperature in ' + new_weather['name'] + ' is ' + str(new_weather['main']['temp'])
                + " Minimum Temperature is " + str(new_weather['main']['temp_min'])
...
def clearAllWeather():
    ...
    "weather.json" each time reset button on the UI 
    is pressed.
    """
    clearWeather = []
    with open(tmpdir + '/weather.json', 'w') as weather_file:
